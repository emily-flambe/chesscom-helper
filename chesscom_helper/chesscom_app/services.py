...
def send_live_match_notification(user, live_games):
    ...
    active_subscriptions = EmailSubscription.objects.filter(player=user, is_active=True)
...
To unsubscribe, please contact the site administrator.
    ...
    for subscription in active_subscriptions:
        try:
            ...
            successful_sends += 1
            logger.info(
                f"Notification sent to {subscription.email} for {user.username}"
            )
            ...
            failed_sends += 1
            logger.error(
                f"Failed to send notification to {subscription.email}: {str(e)}"
            )
...
def check_and_notify_all_users():
    ...
    for user in users:
        try:
            ...
            if (
                not status_result["was_playing"]
                and status_result["is_now_playing"]
                and status_result["status_changed"]
            ):

                notification_result = send_live_match_notification(
                    user, status_result["live_games"]
                )
            ...
            errors.append(
                {
                    "error": f"Error processing {user.username}: {str(e)}",
