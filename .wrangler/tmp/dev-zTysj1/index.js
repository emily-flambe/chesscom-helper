...
// .wrangler/tmp/bundle-HzOvqj/checked-fetch.js
...
// .wrangler/tmp/bundle-HzOvqj/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-HzOvqj/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-HzOvqj/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function validateRequest(request, env) {
  ...
  if (!userAgent) {
    return s(400, "User-Agent header is required");
  }
...
var router5 = t();
router5.all("*", preflight).all("*", rateLimiter).all("*", validateRequest);
router5.all("/api/v1/auth/*", router.fetch);
router5.all("/api/v1/users/*", authenticateUser, router2.fetch);
router5.all("/api/v1/monitoring/*", authenticateUser, router3.fetch);
router5.all("/api/v1/notifications/*", authenticateUser, router4.fetch);
router5.get("/health", () => o({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
router5.all("*", () => s(404, "Not Found"));
var src_default = {
  async fetch(request, env, ctx) {
    try {
      return await router5.handle(request, env, ctx).then(corsify);
    } catch (err) {
  ...
  async scheduled(event, env, ctx) {
    switch (event.cron) {
      case "*/5 * * * *":
        await Promise.resolve().then(() => (init_playerMonitoring(), playerMonitoring_exports)).then((m) => m.checkPlayerStatus(env, ctx));
        break;
      case "0 */6 * * *":
        await Promise.resolve().then(() => (init_cleanup(), cleanup_exports)).then((m) => m.cleanupExpiredTasks(env, ctx));
        break;
    }
  }
};
