...
// .wrangler/tmp/bundle-ikHr0p/checked-fetch.js
    ...
    if (url.pathname === "/api/monitor" && request.method === "POST") {
      try {
        ...
        if (monitoredPlayers.includes(username)) {
          return new Response(
            JSON.stringify({ error: `Already monitoring ${username}` }),
            { status: 400, headers: { "Content-Type": "application/json" } }
          );
        }
...
// .wrangler/tmp/bundle-ikHr0p/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ikHr0p/middleware-loader.entry.ts
